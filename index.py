...
web_app.secret_key = "events-app-key-v1"
...
def aprofile():
     if len(session["email"]) != 0:
        return render_template("a_profile.html", email=session["email"])
     else:
        return redirect("/")
...
@web_app.route("/a_fetch-admin", methods=["POST"])
def fetch_admin_from_db():
    admin_data = {
        "email": request.form["email"],
        "password": request.form["password"],
    }

    db_helper.collection = db_helper.db["admin"]
    try:
        result = db_helper.fetch(query=admin_data)
    except Exception as e:
    ...
    if result:
        ...
        return render_template("a_dashboard.html", email=session['email'])
    else:
